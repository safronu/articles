...
Но это не совсем полный аналог функционального сложения типов, потому что оно подразумевает, что других альтернатив быть не может, что не верно для приведённого кода на Java. Т.е. мы хотим гарантию гарантию, что кроме `StringVersion` и `ByteVersion` других наследников у `SumOfByteAndString` нет.
...
Формально, в Scala, мы используем наследование для реализации 
